...
export default function CreateTeacherCourseAssignment() {
    ...
    const [workloadWarning, setWorkloadWarning] = useState<{ message: string, assignment: any, courseName: string, remainingAssignments: any[], isLastAssignment: boolean } | null>(null);
    ...
    const teacherWorkload = useMemo(() => {
        ...
        const totalAssignedHours = teacherAssignments.reduce((total, assignment) => {
            ...
            if (course.course_type === 'T') { // Theory
                ...
                return total + lectureHours + tutorialHours + (practicalHours * 2);
            } else if (course.course_type === 'L') { // Lab only
                // Practical hours are counted double for workload
                return total + (practicalHours * 2);
            }
    ...
    const calculateLTPHours = (course: Course) => {
        ...
        if (course.course_type === 'T') { // Theory
            ...
            // Practical hours are counted double for workload
            return lectureHours + tutorialHours + (practicalHours * 2);
        } else if (course.course_type === 'L') { // Lab only
            // Practical hours are counted double for workload
            return practicalHours * 2;
        }
    ...
    const courseOptions: ComboboxOption[] = useMemo(() => {
        return availableCourses.map((course: Course) => {
            ...
            const tutorialHours = course.tutorial_hours || 0;
            const practicalHours = course.practical_hours || 0;
            const adjustedPracticalHours = practicalHours * 2;

            return {
                value: course.id.toString(),
                label: `${course.course_detail?.course_name} - ${course.course_year} year - (${course.course_detail?.course_id}) [L:${lectureHours}|T:${tutorialHours}|P:${practicalHours}×2=${adjustedPracticalHours}] - For: ${course.for_dept_detail.dept_name}`
            };
        });
    }, [availableCourses]);
    ...
    // Update process function to properly invalidate queries after each assignment
    const processNextAssignment = async (remainingAssignments: any[]) => {
        if (remainingAssignments.length === 0) {
            // All assignments processed - show final summary toast and refresh data
            if (successCount > 0) {
                toast.success(`Successfully created ${successCount} assignment(s)`);
                // Instead of just refetch, invalidate the queries to ensure fresh data
                await queryClient.invalidateQueries({ queryKey: ['teacherCourseAssignments'] });
                // Also invalidate the teacher workload data
                if (selectedTeacher) {
                    await queryClient.invalidateQueries({
                        queryKey: ['teacherCourseAssignmentsByTeacher', parseInt(selectedTeacher)]
                    });
                }
            }
            return;
        ...
        const [currentAssignment, ...otherAssignments] = remainingAssignments;
        const { assignmentData, courseData } = currentAssignment;

        // Get updated teacher workload data before making decision
        const currentTeacherData = await queryClient.fetchQuery({
            queryKey: ['teacherCourseAssignmentsByTeacher', parseInt(selectedTeacher!)],
            queryFn: async () => {
                try {
                    const response = await api.get(`/api/teacher-courses/teacher/${selectedTeacher}/`);
        ...
        // Calculate teacher's current total workload
        const totalAssignedHours = currentTeacherData.reduce((total: number, assignment: any) => {
            const course = assignment.course_detail;
            if (!course) return total;
        ...
        // Get max hours from selected teacher data
        const maxHours = selectedTeacherData?.teacher_working_hours || 0;
        const availableHours = maxHours - totalAssignedHours;

        // Calculate workload for current course
        const courseWorkload = calculateLTPHours(courseData);

        // Check if this course would exceed teacher's workload limit
        if (availableHours < courseWorkload) {
            // Show confirmation dialog for this assignment
            setWorkloadWarning({
                message: `Assigning "${courseData.course_detail?.course_name}" would exceed ${selectedTeacherData?.teacher_id?.first_name} ${selectedTeacherData?.teacher_id?.last_name}'s workload limit by ${courseWorkload - availableHours} hours.`,
                assignment: assignmentData,
                courseName: courseData.course_detail?.course_name || "Unknown course",
                remainingAssignments: otherAssignments,
                isLastAssignment: otherAssignments.length === 0
            });
            return;
        }

        // If no workload issues, create the assignment directly
        try {
            await createAssignment.mutateAsync(assignmentData);
            setSuccessCount(prev => prev + 1);

            // Invalidate queries after each successful assignment to get fresh workload data
            await queryClient.invalidateQueries({ queryKey: ['teacherCourseAssignments'] });
            if (selectedTeacher) {
                await queryClient.invalidateQueries({
                    queryKey: ['teacherCourseAssignmentsByTeacher', parseInt(selectedTeacher)]
            ...
            // Continue with remaining assignments without showing individual success toasts
            processNextAssignment(otherAssignments);
        } catch (error: any) {
            // Handle error
            setFailureCount(prev => prev + 1);
            const courseName = courseData.course_detail?.course_name || "Unknown course";
            ...
            if (error?.response?.data?.non_field_errors) {
                ...
                errorMessage = `${courseName}: ${error.response.data.detail}`;
            } else if (error?.message) {
                errorMessage = `${courseName}: ${error.message}`;
            }

            toast.error(errorMessage);
            setFailureMessages(prev => [...prev, errorMessage]);

            // Continue with remaining assignments despite this error
            processNextAssignment(otherAssignments);
        }
    };

    // Update workload confirmation handler to also invalidate queries
    const handleWorkloadConfirmation = async (confirm: boolean) => {
        if (!workloadWarning) return;

        if (confirm) {
            try {
                // Show loading toast for this specific assignment
                const loadingToast = toast.loading(`Creating assignment for ${workloadWarning.courseName}...`);

                // Proceed with assignment creation despite workload warning
                const result = await createAssignment.mutateAsync(workloadWarning.assignment);

                // Dismiss loading toast
                toast.dismiss(loadingToast);
                setSuccessCount(prev => prev + 1);

                // Check if result has a warning message
                if (result?.warning) {
                    toast.warning(result.warning);
                }

                // Invalidate queries to refresh workload data
                await queryClient.invalidateQueries({ queryKey: ['teacherCourseAssignments'] });
                if (selectedTeacher) {
                    await queryClient.invalidateQueries({
                        queryKey: ['teacherCourseAssignmentsByTeacher', parseInt(selectedTeacher)]
                ...
                // Don't show individual success toast to avoid notification overload
            } catch (error: any) {
                // Handle error
                setFailureCount(prev => prev + 1);
                const courseName = workloadWarning.courseName;
                let errorMessage = `Failed to assign to ${courseName}`;
            ...
            // User declined to proceed with overloaded assignment
            setFailureCount(prev => prev + 1);
            const courseName = workloadWarning.courseName;
            const message = `${courseName}: Assignment canceled due to workload limit`;
            toast.info(message);
            setFailureMessages(prev => [...prev, message]);
        }

        // Continue with any remaining assignments
        const remainingAssignments = workloadWarning.remainingAssignments || [];

        // Clear the warning state
        setWorkloadWarning(null);

        // Process next assignment (if any)
        processNextAssignment(remainingAssignments);
    };
    ...
    const onSubmit = async (data: TeacherCourseFormValues) => {
        ...
        // Validate other requirements (not workload - we'll handle that with warnings)
        ...
        // Create assignments in sequence with more explicit control
        try {
            // Prepare all assignments first
            const assignmentQueue = data.assignments.map(assignment => {
                const courseData = courses.find(c => c.id.toString() === assignment.course_id);
                if (!courseData) return null;
                ...
                return {
                    assignmentData: {
                        ...
                        semester: courseData.course_semester || 1,
                        academic_year: courseData.course_year || new Date().getFullYear(),
            ...
            // Start processing assignments one by one
            toast.info(`Processing ${assignmentQueue.length} assignment(s)...`);
            processNextAssignment(assignmentQueue);
        } catch (error: any) {
    ...
    return (
        <>
                                            ...
                                            <div className="space-y-2">
                                                {teacherWorkload.assignments.map(assignment => {
                                                    ...
                                                    if (course?.course_type === 'T') {
                                                        ...
                                                        ltpHours = lectureHours + tutorialHours + (practicalHours * 2);
                                                    } else if (course?.course_type === 'L') {
                                                        ltpHours = practicalHours * 2;
                                                    } else {
                                                        ltpHours = course?.credits || 0;
                                                    }
                                                    return (
                                                        <div key={assignment.id} className="flex justify-between items-center p-2 bg-muted/30 rounded-md">
                                                            <div>
                                                                ...
                                                                <p className="text-xs text-muted-foreground">
                                                                    L:{lectureHours} T:{tutorialHours} P:{practicalHours}×2={practicalHours * 2}
                                                                </p>
                                                            </div>
                                        ...
                                        <div key={courseId} className="border rounded-lg p-3">
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-1 mt-1">
                                                        ...
                                                        <Badge variant="outline" className="text-xs bg-primary/5">
                                                            L:{lHours} T:{tHours} P:{pHours}×2={pHours * 2} ({totalHours}h)
                                                        </Badge>
                                                        <Badge variant="outline" className="text-xs">
                                                ...
                                                {courseAssignments.length > 0 && (
                                                    <Badge variant="outline" className="ml-2">
                                                        {courseAssignments.length} teacher(s)
                                                    </Badge>
                                                )}
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mb-2">
                                                <div className="bg-secondary/10 p-2 rounded-md flex items-center justify-between">
                                                    ...
                                                    <div className={courseAssignments.length > 0 ? "text-green-600 text-xs" : "text-muted-foreground text-xs"}>
                                                        {courseAssignments.length > 0 ? "Assigned" : "None assigned"}
                                                    </div>
                                                </div>
            ...
            <AlertDialog open={!!workloadWarning} onOpenChange={(open) => !open && setWorkloadWarning(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Workload Limit Exceeded</AlertDialogTitle>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel onClick={() => handleWorkloadConfirmation(false)}>Cancel Assignment</AlertDialogCancel>
                        <AlertDialogAction onClick={() => handleWorkloadConfirmation(true)}>Proceed Anyway</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
        </>
