...
class StudentCourseViewSet(viewsets.ModelViewSet):
    ...
    def available_courses(self, request):
        ...
        try:
            ...
            logger.info(f"User {request.user.pk} requested available_courses")
            ...
            if not student:
                logger.warning(f"No student profile found for user {request.user.pk}")
                return Response(
            ...
            if not has_dept:
                logger.warning(f"Student {student.pk} missing department")
                return Response(
            ...
            if not has_semester:
                logger.warning(f"Student {student.pk} missing current semester")
                return Response(
                    {"detail": "Your student profile does not have a current semester set"},
                    status=status.HTTP_400_BAD_REQUEST
                )
                
            logger.info(f"Getting courses for dept_id={student.dept_id.pk}, semester={student.current_semester}")

            # Get courses for student's department, semester, and year
            courses = Course.objects.filter(
                for_dept_id=student.dept_id,
                course_semester=student.current_semester,
                course_year=student.current_year
            ).select_related('course_id', 'for_dept_id', 'teaching_dept_id', 'course_master')
    ...
    def available_courses_all(self, request):
        ...
        try:
            logger = logging.getLogger('django')
            logger.info(f"User {request.user.pk} requested available_courses_all")
